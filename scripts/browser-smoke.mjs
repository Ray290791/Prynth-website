...
const outPng = checkedOutputPath(args.outPng);
const derived = derivedPaths(outPng);
const mobilePng = checkedOutputPath(derived.mobilePng);
const outJson = checkedOutputPath(derived.verdictJson, undefined, "verdict JSON");
...
if (baselineRequested) {
  try {
    baselinePath = checkedOutputPath(realpathSync(args.baseline), undefined, "baseline");
  } catch (err) {
